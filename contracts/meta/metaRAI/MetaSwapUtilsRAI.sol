...
import "../../interfaces/ISnappedRedemptionPrice.sol";
...
library MetaSwapUtilsRAI {
    ...
    //PRECISION OF ALL RATES USED (Base Virtual Price and RAI Redemption Price)
    uint256 public constant RATES_PRECISION = 10**18;
    ...
    /**
     * @notice retrieve the current RAI redemption price and scale to RATES_PERCISION
    * @param metaSwapStorage MetaSwap struct to read from
     * @return the current RAI redemption price scaled to RATES_PERCISION
     */
    function _getScaledRedemptionPrice(
        MetaSwap storage metaSwapStorage
    )
        internal
        view
        returns (uint256)
    {
        uint256 rate = ISnappedRedemptionPrice(metaSwapStorage.redemptionPriceFeed).snappedRedemptionPrice();
        require((metaSwapStorage.redemptionPricePrecisionDiff)>0,"redemption price precision diff = 0");
        //console.log("rate is %s scaling is %s", rate, metaSwapStorage.redemptionPricePrecisionDiff);
        return rate.div(10**uint256(metaSwapStorage.redemptionPricePrecisionDiff));
    }
     ...
     * @param rates struct containing the virtual price of the base swap's LP token and RAI's redemption price
        ...
        )
    {
        ...
        uint256[] memory xp = _xp_mem(self, v.rates);
        ...
        if (tokenIndex == xp.length.sub(1)) {
            dy = dy.mul(RATES_PRECISION).div(v.rates.baseVirtualPrice);
        }
        else{
            dy = dy.mul(RATES_PRECISION).div(v.rates.redemptionPrice);
        }
    ...
    /**
     * @notice Given a set of balances, precision multipliers and rates, return the
     * precision-and-rate-adjusted balances.
     * @param balances an array of token balances, in their native precisions.
     * These should generally correspond with pooled tokens.
     *
     * @param precisionMultipliers an array of multipliers, corresponding to
    ...
    function _xp_mem(
        ...
        uint256[] memory xp =  SwapUtils._xp(balances, precisionMultipliers);
        uint256 baseLPTokenIndex = balances.length.sub(1);
        xp[baseLPTokenIndex] = balances[baseLPTokenIndex].mul(rates.baseVirtualPrice).div(RATES_PRECISION);
        xp[0] = balances[0].mul(rates.redemptionPrice).div(RATES_PRECISION);   
    ...
    */
     ...
     * @param self Swap struct to read from
     * @param metaSwapStorage MetaSwap struct to read from
     * @return the virtual price, scaled to precision of RATES_PRECISION
     */
    function getVirtualPrice(
        ...
        uint256 numTokens = self.balances.length;
        ...
        if (supply != 0) {
            return d.mul(RATES_PRECISION).div(supply);
        }
        return 0;
    }
    ...
    function _calculateSwap(
        ...
        uint256[] memory xp = _xp_mem(self, rates);
        require(
            tokenIndexFrom < xp.length && tokenIndexTo < xp.length,
            "Token index out of range"
        );
        uint256 x = dx.mul(self.tokenPrecisionMultipliers[tokenIndexFrom]).mul(rates.redemptionPrice).div(RATES_PRECISION).add(
            xp[tokenIndexFrom]
        );
    ...
    function calculateSwapUnderlying(
        ...
        if (tokenIndexFrom < v.baseLPTokenIndex) {
            ...
            v.x = xp[tokenIndexFrom].add(  
                dx.mul(self.tokenPrecisionMultipliers[tokenIndexFrom]).mul(v.rates.redemptionPrice).div(RATES_PRECISION)
            );
        } else {
        ...
        if (tokenIndexTo < v.baseLPTokenIndex) {
            ...
            v.dy = v.dy.div(self.tokenPrecisionMultipliers[v.metaIndexTo]).mul(RATES_PRECISION).div(v.rates.redemptionPrice);
        } else {
            // tokenTo is from the base pool
            v.dy = v.baseSwap.calculateRemoveLiquidityOneToken(
                v.dy.mul(RATES_PRECISION).div(v.rates.baseVirtualPrice),
                tokenIndexTo - v.baseLPTokenIndex
            );
        }
    ...
    function calculateTokenAmount(
        ...
        uint256 d1;
        {
            RatesInfo memory RI = _getRates(metaSwapStorage);
            ...
            d0 = SwapUtils.getD(
                _xp_mem(balances1, tokenPrecisionMultipliers, RI),
                a
            );
    ...
    function _getRates(
        MetaSwap storage metaSwapStorage
    ) internal view returns (RatesInfo memory) {
        RatesInfo memory RI = RatesInfo(
            _getBaseVirtualPrice(metaSwapStorage),
            _getScaledRedemptionPrice(metaSwapStorage)
        );
        return RI;
    }
    ...
    function swapUnderlying(
        ...
        if (
            ...
            if (tokenIndexFrom < baseLPTokenIndex) {
                ...
                v.x = xp[tokenIndexFrom].add(
                    dx.mul(v.tokenPrecisionMultipliers[tokenIndexFrom]).mul(v.rates.redemptionPrice).div(RATES_PRECISION)
                );
            } else {
            ...
            if (tokenIndexTo >= baseLPTokenIndex) {
                ...
                v.dy = v.dy.mul(RATES_PRECISION).div(
                    v.rates.baseVirtualPrice
                );
            }
    ...
    function addLiquidity(
        ...
        for (uint256 i = 0; i < pooledTokens.length; i++) {
            //console.log("total supply is %s ",v.totalSupply);
            //console.log("i is  %s amount is %s",i,amounts[i]);
            require(
        ...
        v.d1 = SwapUtils.getD(
            _xp_mem(v.newBalances, v.tokenPrecisionMultipliers, v.rates),
            v.preciseA
        );
        
        //console.log("D returned: %s original D",v.d1, v.d0);
    ...
    function removeLiquidityImbalance(
        ...
        uint256[] memory fees = new uint256[](v.newBalances.length);
        {
            ...
            v.d1 = SwapUtils.getD(
                _xp_mem(balances1, v.tokenPrecisionMultipliers, v.rates),
                v.preciseA
            );
