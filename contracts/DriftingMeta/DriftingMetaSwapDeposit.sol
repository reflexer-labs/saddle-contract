...
import "../interfaces/IDriftingMetaSwap.sol";

/**
 * @title DriftingMetaSwapDeposit
 * @notice This contract flattens the LP token in a DriftingMetaSwap pool for easier user access. DriftingMetaSwap must
 * be deployed before this contract can be initialized successfully.
 *
 * For example, suppose there exists a base Swap pool consisting of [DAI, USDC, USDT].
 * Then a DriftingMetaSwap pool can be created with [RAI, BaseSwapLPToken] to allow trades between either
 * the LP token or the underlying tokens and RAI.
 *
 * DriftingMetaSwapDeposit flattens the LP token and remaps them to a single array, allowing users
 * to ignore the dependency on BaseSwapLPToken. Using the above example, DriftingMetaSwapDeposit can act
 * as a Swap containing [RAI, DAI, USDC, USDT] tokens.
 */
contract DriftingMetaSwapDeposit is Initializable, ReentrancyGuardUpgradeable {
    using SafeERC20 for IERC20;
    using SafeMath for uint256;

    ISwap public baseSwap;
    IDriftingMetaSwap public driftingMetaSwap;
    ...
    struct RemoveLiquidityImbalanceInfo {
        ISwap baseSwap;
        IDriftingMetaSwap driftingMetaSwap;
        IERC20 metaLPToken;
    ...
    /**
     * @notice Sets the address for the base Swap contract, DriftingMetaSwap contract, and the
     * DriftingMetaSwap LP token contract.
     * @param _baseSwap the address of the base Swap contract
     * @param _driftingMetaSwap the address of the DriftingMetaSwap contract
     * @param _metaLPToken the address of the DriftingMetaSwap LP token contract
     */
    function initialize(
        ISwap _baseSwap,
        IDriftingMetaSwap _driftingMetaSwap,
        IERC20 _metaLPToken
        ...
        // Check and approve base level tokens to be deposited to the base Swap contract
        {
            ...
            for (; i < 32; i++) {
                try _baseSwap.getToken(i) returns (IERC20 token) {
                    ...
                    token.safeApprove(address(_driftingMetaSwap), MAX_UINT256);
                } catch {
                    break;
                }
            }
        ...
        // Check and approve meta level tokens to be deposited to the DriftingMetaSwap contract
        IERC20 baseLPToken;
        {
            uint8 i;
            for (; i < 32; i++) {
                try _driftingMetaSwap.getToken(i) returns (IERC20 token) {
                    baseLPToken = token;
            ...
            require(i > 1, "driftingMetaSwap must have at least 2 tokens");
        }
        ...
        // Approve DriftingMetaSwap LP token to be burned by the DriftingMetaSwap contract for withdrawing
        _metaLPToken.safeApprove(address(_driftingMetaSwap), MAX_UINT256);

        // Initialize storage variables
        baseSwap = _baseSwap;
        driftingMetaSwap = _driftingMetaSwap;
    ...
    function swap(
        ...
        uint256 tokenToAmount = driftingMetaSwap.swapUnderlying(
            tokenIndexFrom,
            tokenIndexTo,
            dx,
            minDy,
    ...
    function addLiquidity(
        ...
        uint256 metaLPTokenAmount;
        {
            ...
            metaLPTokenAmount = driftingMetaSwap.addLiquidity(
                metaAmounts,
                minToMint,
                deadline
            );
    ...
    function removeLiquidity(
        ...
        uint256 baseLPTokenAmount;
        {
            // Remove liquidity from the DriftingMetaSwap pool
            ...
            uint256 baseLPTokenIndex = memMetaTokens.length - 1;
            {
                ...
                removedAmounts = driftingMetaSwap.removeLiquidity(
                    amount,
                    metaMinAmounts,
                    deadline
                );
    ...
    function removeLiquidityOneToken(
        ...
        if (tokenIndex < baseLPTokenIndex) {
            ...
            uint256 removedBaseLPTokenAmount = driftingMetaSwap.removeLiquidityOneToken(
                tokenAmount,
                baseLPTokenIndex,
                0,
                deadline
    ...
    function removeLiquidityImbalance(
        ...
        // Transfer DriftingMetaSwap LP token from the caller to this contract
        ...
        // Withdraw the paired meta level tokens and the base LP token from the DriftingMetaSwap pool
        uint256 burnedMetaLPTokenAmount = v.driftingMetaSwap.removeLiquidityImbalance(
            metaAmounts,
            maxBurnAmount,
            deadline
        );
        ...
        if (v.withdrawFromBase) {
            ...
            // In that case, deposit it to the DriftingMetaSwap pool.
            ...
            if (leftoverBaseLPTokenAmount > 0) {
                ...
                v.leftoverMetaLPTokenAmount = v.leftoverMetaLPTokenAmount.add(
                    v.driftingMetaSwap.addLiquidity(leftovers, 0, deadline)
                );
            }
        }
        ...
        returns (uint256)
    {
        ...
        return driftingMetaSwap.calculateTokenAmount(metaAmounts, deposit);
    }
    ...
    {
        uint256[] memory metaAmounts = driftingMetaSwap.calculateRemoveLiquidity(
            amount
        );
    ...
    function calculateRemoveLiquidityOneToken(
        ...
        if (tokenIndex < baseLPTokenIndex) {
            ...
            uint256 baseLPTokenAmount = driftingMetaSwap
    ...
    function calculateSwap(
        ...
        return
            driftingMetaSwap.calculateSwapUnderlying(tokenIndexFrom, tokenIndexTo, dx);
    }
}
