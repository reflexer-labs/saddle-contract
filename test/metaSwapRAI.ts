    ...
    describe("Check for attacks while A is ramping upwards", () => {
      ...
      /*describe(
        ...
        () => {
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 1e18 of RAI to lpToken, causing massive imbalance in the pool
            await metaSwapRAI
              .connect(attacker)
            ...
            expect(secondTokenOutput).to.be.eq("986227857942205817")

            // Pool is imbalanced! Now trades from lpToken -> RAI may be profitable in small sizes
            // RAI balance in the pool  : 2.00e18
            // lpToken balance in the pool : 9.14e16
            expect(await metaSwapRAI.getTokenBalance(0)).to.be.eq(String(2e18))
            expect(await metaSwapRAI.getTokenBalance(1)).to.be.eq(
              "13772142057794183",
            )
            ...
            // Trade lpToken to RAI, taking advantage of the imbalance and change of A
            const balanceBefore = await getUserTokenBalance(attacker, RAI)
            await metaSwapRAI
              .connect(attacker)
              .swap(1, 0, secondTokenOutput, 0, MAX_UINT256)
            const firstTokenOutput = (
              await getUserTokenBalance(attacker, RAI)
            ).sub(balanceBefore)

            // If firstTokenOutput > 1e18, the malicious user leaves with more RAI than the start.
            ...
            // Attacker lost 2.785e15 RAI (0.2785% of initial deposit)

            // Check for pool balance changes
            const finalPoolBalances = []
            finalPoolBalances.push(await metaSwapRAI.getTokenBalance(0))
            finalPoolBalances.push(await metaSwapRAI.getTokenBalance(1))
            ...
            // Pool (liquidity providers) gained 2.785e15 RAI (0.2785% of RAI balance)
          ...
          it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Trade lpToken to RAI, taking advantage of the imbalance and sudden change of A
            const balanceBefore = await getUserTokenBalance(attacker, RAI)
            await metaSwapRAI
              .connect(attacker)
              .swap(1, 0, secondTokenOutput, 0, MAX_UINT256)
            ...
            // Attacker lost 4.426e16 RAI (4.426%)
            ...
            // Pool (liquidity providers) gained 4.426e16 RAI (4.426% of RAI balance of the pool)
            // The attack did not benefit the attacker.
          })
        },
      )*/
      ...
      /*describe(
        ...
        () => {
          ...
          it("Attack fails with 900 seconds between blocks", async () => {
            // Swap 1e18 of RAI to lpToken, resolving imbalance in the pool
            await metaSwapRAI
              .connect(attacker)
              .swap(0, 1, String(1e18), 0, MAX_UINT256)
            const secondTokenOutput = (
            ...
            expect(secondTokenOutput).to.be.eq("2725775227635006918")

            // Pool is now almost balanced!
            // RAI balance in the pool  : 2.000e18
            ...
            // If firstTokenOutput > 1e18, the attacker leaves with more RAI than the start.
            ...
            // Attacker lost 1.982e15 RAI (0.1982% of initial deposit)
            ...
            // Pool (liquidity providers) gained 1.982e15 RAI (0.1982% of RAI balance)
            // The attack did not benefit the attacker.
          })
          ...
          it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Swap 1e18 of RAI to lpToken, resolving the imbalance in the pool
            await metaSwapRAI
              .connect(attacker)
              .swap(0, 1, String(1e18), 0, MAX_UINT256)
            const secondTokenOutput = (
            ...
            // Attacker gained 4.430e15 RAI (0.430%)
            ...
            // Pool (liquidity providers) lost 4.430e15 RAI (0.430% of RAI balance)
      ...
      describe(
        ...
        () => {
          ...
          /*it("Attack fails with 900 seconds between blocks", async () => {
            ...
            // Attacker lost 2.723e15 RAI (0.2723% of initial deposit)
            ...
            // Pool (liquidity providers) gained 2.723e15 RAI (0.2723% of RAI balance)
            // The attack did not benefit the attacker.
          })*/
          ...
          /*it("Attack succeeds with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Attacker gained 6.625e16 RAI (6.625% of initial deposit)
            ...
            // Pool (liquidity providers) lost 6.625e16 RAI (6.625% of RAI balance)
      ...
      describe(
        ...
        () => {
          ...
          /*it("Attack fails with 900 seconds between blocks", async () => {
            ...
            // Attacker lost 1.992e15 RAI (0.1992% of initial deposit)
            ...
            // Pool (liquidity providers) gained 1.992e15 RAI (0.1992% of RAI balance)
            // The attack did not benefit the attacker.
          })*/
          ...
          /*it("Attack fails with 2 weeks between transactions (mimics rapid A change)", async () => {
            ...
            // Attacker lost 1.368e16 RAI (1.368% of initial deposit)
            ...
            // Pool (liquidity providers) gained 1.368e16 RAI (1.368% of RAI balance)
            // The attack did not benefit the attacker
          })*/
        },
      )
